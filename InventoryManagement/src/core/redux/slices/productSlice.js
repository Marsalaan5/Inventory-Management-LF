...
//         const index = state.product_list.findIndex(p => p.prod_id === action.payload.prod_id);
//         if (index !== -1) {
...
//         state.product_list = state.product_list.filter(p => p.prod_id !== action.payload);
//       })
...
//     setFilters: (state, action) => {
//       state.filters = { ...state.filters, ...action.payload };
//       // Reset to page 1 when non-pagination filters change
...
// // i want a bulk upload option like from excel sheet,, format will be exactly same as add product and everything will be same same like barcode generation and all  like if i got a sheet of 1000 or more and i want to add product  and shoulkd show in add product list ...
// export const updateProduct = createAsyncThunk(
//   'products/updateProduct',
//   async ({ prod_id, data }, { rejectWithValue }) => {
//     try {
...
//           state.product_list = [...action.payload.data, ...state.product_list];
//         }
//       })
...
//with bulk product update using lot id 
